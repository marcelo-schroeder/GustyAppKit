...
#import "IFA_DTWeakSupport.h"

@class IFA_DTHTMLParser;
/** The IFA_DTHTMLParserDelegate protocol defines the optional methods implemented by delegates of IFA_DTHTMLParser objects.
 
 Dependencies: libxml2.dylib
 */
@protocol IFA_DTHTMLParserDelegate <NSObject>
 ...
 */
- (void)parserDidStartDocument:(IFA_DTHTMLParser *)parser;
...
/**
 ...
 @param parser A parser object.
 */
- (void)parserDidEndDocument:(IFA_DTHTMLParser *)parser;
...
/**
 ...
 @param parser A IFA_DTHTMLParser object parsing HTML.
 @param comment A string that is a the content of a comment in the XML.
 */
- (void)parser:(IFA_DTHTMLParser *)parser foundComment:(NSString *)comment;
 ...
 */
- (void)parser:(IFA_DTHTMLParser *)parser foundCDATA:(NSData *)CDATABlock;
 ...
 */
- (void)parser:(IFA_DTHTMLParser *)parser foundProcessingInstructionWithTarget:(NSString *)target data:(NSString *)data;
 ...
 */
- (void)parser:(IFA_DTHTMLParser *)parser parseErrorOccurred:(NSError *)parseError;

@end

/** Instances of this class parse HTML documents (including DTD declarations) in an event-driven manner. A IFA_DTHTMLParser notifies its delegate about the items (elements, attributes, CDATA blocks, comments, and so on) that it encounters as it processes an HTML document. It does not itself do anything with those parsed items except report them. It also reports parsing errors. For convenience, an IFA_DTHTMLParser object in the following descriptions is sometimes referred to as a parser object.
 */
@interface IFA_DTHTMLParser : NSObject
...
/**
 ...
 @returns An initialized `IFA_DTHTMLParser` object or nil if an error occurs.
 */
...
/**
 ...
 @param delegate An object that is the new delegate. It is not retained. The delegate must conform to the IFA_DTHTMLParserDelegate Protocol protocol.
 
 @see delegate
 */
- (void)setDelegate:(id <IFA_DTHTMLParserDelegate>)delegate;
...
/**
 ...
 @see delegate
 */
- (id <IFA_DTHTMLParserDelegate>)delegate;

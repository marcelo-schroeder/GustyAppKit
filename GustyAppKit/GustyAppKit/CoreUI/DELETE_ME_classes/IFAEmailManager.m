...
//            NSLog(@"a_attachmentUrl.lastPathComponent: %@", a_attachmentUrl.lastPathComponent);
        ...
        [IFAUIUtils showAlertWithMessage:NSLocalizedStringFromTable(@"This device is not able to send email!", @"GustyAppKitLocalizable", nil) title:NSLocalizedStringFromTable(@"Warning", @"GustyAppKitLocalizable", nil)];
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error{
    ...
    if (result==MFMailComposeResultFailed) {
        [IFAUtils dispatchAsyncMainThreadBlock:^{
            [IFAUIUtils showAlertWithMessage:NSLocalizedStringFromTable(@"The email has not been sent due to an error.", @"GustyAppKitLocalizable", nil) title:NSLocalizedStringFromTable(@"Email Error", @"GustyAppKitLocalizable", nil)];
        }                           afterDelay:0.1];
    }

...
- (IFASingleSelectionListViewControllerHeaderView *)customHeaderView {
    if (!_customHeaderView) {
        ...
        if (self.disallowDeselection) {
            text = NSLocalizedStringFromTable(@"Tap any entry below to select it", @"GustyAppKitLocalizable", nil);
        } else {
            text = NSLocalizedStringFromTable(@"Tap any entry below to select or deselect it", @"GustyAppKitLocalizable", nil);
        }

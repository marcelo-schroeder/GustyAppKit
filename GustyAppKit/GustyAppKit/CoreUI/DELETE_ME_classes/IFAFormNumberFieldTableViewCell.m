...
-(BOOL)textFieldShouldEndEditing:(UITextField *)textField{
    ...
    if (self.formViewController.textFieldCommitSuspended) {
        ...
        if ([self parsedValue]) {
            ...
            NSString *alertTitle = NSLocalizedStringFromTable(@"Validation Error", @"GustyAppKitLocalizable", nil);
            NSString *alertMessage = [NSString stringWithFormat:NSLocalizedStringFromTable(@"Invalid number entered for %@.", @"GustyAppKitLocalizable", @"Invalid number entered for <FIELD_NAME>."), l_propertyLabel];

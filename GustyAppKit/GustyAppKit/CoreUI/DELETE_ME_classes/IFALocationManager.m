...
+ (void)showLocationServicesAlertWithMessage:(NSString *)a_message
    ...
    NSString *title = NSLocalizedStringFromTable(@"Unable to obtain your location", @"GustyAppKitLocalizable", nil);
    ...
    if (a_shouldShowSettingsOption) {
        ...
        [l_alertActions addObject:[UIAlertAction actionWithTitle:NSLocalizedStringFromTable(@"Settings", @"GustyAppKitLocalizable", nil)
                                                           style:UIAlertActionStyleDefault
                                                         handler:settingsHandlerBlock]];
    } else {
        [l_alertActions addObject:[UIAlertAction actionWithTitle:NSLocalizedStringFromTable(@"Continue", @"GustyAppKitLocalizable", nil)
                                                           style:UIAlertActionStyleDefault
                                                         handler:nil]];
    }
...
+ (void)handleLocationFailureWithAlertPresenterViewController:(UIViewController *)a_alertPresenterViewController {
    if ([self performLocationServicesChecksWithAlertPresenterViewController:a_alertPresenterViewController]) {
        NSString *message = NSLocalizedStringFromTable(@"Please check if your device has Internet connectivity.", @"GustyAppKitLocalizable", nil);
...
performLocationServicesChecksWithAlertPresenterViewController:(UIViewController *)a_alertPresenterViewController {
    if (![CLLocationManager locationServicesEnabled]) {
        NSString *message = NSLocalizedStringFromTable(@"Location Services are currently disabled. Please enable them in the Privacy section of the Settings app.", @"GustyAppKitLocalizable", nil);
        [self showLocationServicesAlertWithMessage:message
                           presenterViewController:a_alertPresenterViewController];
        return NO;
    }
    ...
    switch ([CLLocationManager authorizationStatus]) {
        ...
        {
            NSString *message = NSLocalizedStringFromTable(@"Your device is not authorised to use Location Services.", @"GustyAppKitLocalizable", nil);
            [self showLocationServicesAlertWithMessage:message
                               presenterViewController:a_alertPresenterViewController];
            return NO;
        }
        ...
        {
            NSString *message = NSLocalizedStringFromTable(@"Location access is currently disabled for this app. Please enable it in the Location section of Settings. Tap the Settings button below to open Settings.", @"GustyAppKitLocalizable", nil);

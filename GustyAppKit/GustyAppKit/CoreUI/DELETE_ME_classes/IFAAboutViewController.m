...
- (IBAction)bugReportButtonTap:(id)sender{
    NSString *l_body = [NSString stringWithFormat:NSLocalizedStringFromTable(@"Hi there,\n\nPlease fix the following bug I have found in %@:", @"GustyAppKitLocalizable", @"Hi there, Please fix the following bug I have found in <APP_NAME>:"), [IFAUtils appFullName]];
    [self.IFA_emailManager composeEmailWithSubject:NSLocalizedStringFromTable(@"Bug Report", @"GustyAppKitLocalizable", nil) recipient:[self IFA_bugReportEmailAddress] body:l_body];
}

- (IBAction)feedbackButtonTap:(id)sender{
    NSString *l_body = [NSString stringWithFormat:NSLocalizedStringFromTable(@"Hi there,\n\nI have the following feedback to provide for %@:", @"GustyAppKitLocalizable", @"Hi there, I have the following feedback to provide for <APP_NAME>:"), [IFAUtils appFullName]];
    [self.IFA_emailManager composeEmailWithSubject:NSLocalizedStringFromTable(@"Feedback", @"GustyAppKitLocalizable", nil) recipient:[self IFA_feedbackEmailAddress] body:l_body];
}
...
- (IBAction)thirdPartyCreditsButtonTap:(id)sender {
    ...
    l_viewController.title = NSLocalizedStringFromTable(@"Third Party Credits", @"GustyAppKitLocalizable", nil);

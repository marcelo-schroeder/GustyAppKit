	...
	for (int i = 0; i < [aKeyArray count]; i++) {
		if (i>0) {
			if (i==([aKeyArray count]-1)) {
				[label appendString:NSLocalizedStringFromTable(@" and ", @"GustyAppKitLocalizable", @"final separator in a list of names")];
			}else {
				[label appendString:NSLocalizedStringFromTable(@", ", @"GustyAppKitLocalizable", @"separator in a list of names")];
			}

...
- (void)dialPhoneNumber:(NSString *)a_phoneNumber {
    if ([self IFA_isPhoneServiceAvailable]) {
        ...
        l_alertMessage = [NSString stringWithFormat:NSLocalizedStringFromTable(@"Please call %@ using a phone", @"GustyAppKitLocalizable", @"Please call <PHONE_NUMBER> using a phone"), l_formattedPhoneNumber];
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:l_alertTitle message:l_alertMessage delegate:self cancelButtonTitle:NSLocalizedStringFromTable(@"OK", @"GustyAppKitLocalizable", nil) otherButtonTitles:nil, nil];
        [alert show];
    }
}

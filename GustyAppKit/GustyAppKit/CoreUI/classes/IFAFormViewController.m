...
- (UITableViewCell *)IFA_staticFieldTypeCellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    switch (l_fieldType) {
        ...
        case IFAEntityConfigFieldTypeButton: {
            ...
            if (l_isDeleteButton) {
                NSString *l_text = NSLocalizedStringFromTable(@"Delete", @"GustyAppKitLocalizable", nil);
                NSString *l_entityLabel = [self.object.ifa_entityLabel lowercaseString];
                if (l_entityLabel) {
                    l_text= [NSString stringWithFormat:NSLocalizedStringFromTable(@"Delete %@", @"GustyAppKitLocalizable", @"Delete <ENTITY_NAME>"), l_entityLabel];
                }
    ...
    if (self.IFA_shouldUsePersistenceChangeDetector) {
        ...
        if ([a_persistenceChangeDetector changedForManagedObject:a_managedObject]) {
            ...
            [IFAUtils dispatchAsyncMainThreadBlock:^{
                ...
                NSString *title = [NSString stringWithFormat:NSLocalizedStringFromTable(@"%@ modified externally", @"GustyAppKitLocalizable", @"ENTITY_LABEL modified externally"),
                                                             objectLabel];
                NSString *message = NSLocalizedStringFromTable(@"Editing will be cancelled and changes discarded.", @"GustyAppKitLocalizable", nil);
                void (^actionBlock)() = ^{
    ...
    if (self.editing) {
        if (!a_forced && self.IFA_isManagedObject && ([IFAPersistenceManager sharedInstance].isCurrentManagedObjectDirty || self.IFA_textFieldTextChanged)) {
            ...
            [self ifa_presentAlertControllerWithTitle:nil
                                              message:NSLocalizedStringFromTable(@"Are you sure you want to discard your changes?", @"GustyAppKitLocalizable", nil)
                         destructiveActionButtonTitle:NSLocalizedStringFromTable(@"Discard changes", @"GustyAppKitLocalizable", nil)
        ...
        if ([a_cell isMemberOfClass:[IFASwitchTableViewCell class]]) {
            ...
            if ([a_cell isKindOfClass:[IFAFormTextFieldTableViewCell class]]) {
                ...
                if (self.IFA_isManagedObject) {
                    ...
                    l_cell.textField.placeholder = propertyDescription.isOptional ? NSLocalizedStringFromTable(@"Optional", @"GustyAppKitLocalizable", nil) : NSLocalizedStringFromTable(@"Required", @"GustyAppKitLocalizable", nil);
                }
                [l_cell reloadData];
    ...
    switch (l_fieldType) {
        ...
        case IFAEntityConfigFieldTypeButton: {
            ...
            if ([l_cell.propertyName isEqualToString:IFAEntityConfigPropertyNameDeleteButton]) {
                ...
                NSString *l_message = [NSString stringWithFormat:NSLocalizedStringFromTable(@"Are you sure you want to delete the %@?", @"GustyAppKitLocalizable", @"Are you sure you want to delete the <ENTITY_NAME>?"), l_entityName];
                NSString *l_destructiveActionButtonTitle = [NSString stringWithFormat:NSLocalizedStringFromTable(@"Delete %@", @"GustyAppKitLocalizable", @"Delete <ENTITY_NAME>"), l_entityName];
                __weak __typeof(self) l_weakSelf = self;
                void (^destructiveActionBlock)() = ^{
                    ...
                    [IFAUIUtils showAndHideUserActionConfirmationHudWithText:[NSString stringWithFormat:NSLocalizedStringFromTable(@"%@ deleted", @"GustyAppKitLocalizable", @"<ENTITY_NAME> deleted"),
                                                                                                        l_weakSelf.title]];
                };
                [self ifa_presentAlertControllerWithTitle:nil
                                                  message:l_message
        ...
        if (!self.isSubForm) {   // does not execute this block if it's a context switching scenario for a sub-form
            ...
            if (self.IFA_isManagedObject) {
                ...
                if ([l_managedObject isInserted] || [l_managedObject isUpdated]) {
                    ...
                    NSString *hudTextFormat = NSLocalizedStringWithDefaultValue(@"ActionPerformedOnObject", @"GustyAppKitLocalizable", [NSBundle bundleForClass:[self class]], @"%@ %@", @"Example: <Project> <updated>");
                    NSString *hudText = [NSString stringWithFormat:hudTextFormat,
                                                                   self.title,
                                    l_isInserted ? NSLocalizedStringFromTable(@"created", @"GustyAppKitLocalizable", nil) : NSLocalizedStringFromTable(@"updated", @"GustyAppKitLocalizable", nil)];

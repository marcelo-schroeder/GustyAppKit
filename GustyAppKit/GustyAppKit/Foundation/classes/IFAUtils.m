...
+ (void) logBooleanWithLabel:(NSString*)a_label value:(BOOL)a_value{
	...
	NSLog(l_label, a_value ? NSLocalizedStringFromTable(@"YES", @"GustyAppKitLocalizable", nil) : NSLocalizedStringFromTable(@"NO", @"GustyAppKitLocalizable", nil));
}
...
+(NSString*)appVersionAndBuildNumber{
    return [NSString stringWithFormat:NSLocalizedStringFromTable(@"%@ (build %@)", @"GustyAppKitLocalizable", @"<APP_VERSION> (build <BUILD_NUMBER>)"), [self appVersion], [IFAUtils appBuildNumber]];
}

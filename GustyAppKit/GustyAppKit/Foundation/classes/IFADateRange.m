    ...
    if (aFormat== IFADurationFormatDecimalHours) {
        ...
        if (aFormat== IFADurationFormatFull || aFormat== IFADurationFormatHoursMinutesSeconds || aFormat== IFADurationFormatHoursMinutes) {
            ...
            switch (aFormat) {
                case IFADurationFormatFull:
                    if (days>0) {
                        l_durationString = [NSString stringWithFormat:NSLocalizedStringFromTable(@"%@d %@:%@:%@", @"GustyAppKitLocalizable", @"<NUMBER_OF_DAYS>d <HOURS>:<MINUTES>:<SECONDS>"), l_formattedDays, l_formattedHours, l_formattedMinutes, l_formattedSeconds];
                        break;
                    }
                case IFADurationFormatHoursMinutesSeconds:
                    l_durationString = [NSString stringWithFormat:NSLocalizedStringFromTable(@"%@:%@:%@", @"GustyAppKitLocalizable", @"<HOURS>:<MINUTES>:<SECONDS>"), l_formattedHours, l_formattedMinutes, l_formattedSeconds];
                    break;
                case IFADurationFormatHoursMinutes:
                    l_durationString = [NSString stringWithFormat:NSLocalizedStringFromTable(@"%@:%@", @"GustyAppKitLocalizable", @"<HOURS>:<MINUTES>"), l_formattedHours, l_formattedMinutes];
                    break;
                default:
                    NSAssert1(NO, @"Unexpected duration format: %ld", (long)aFormat);
            }
            ...
            if (showDay) {
                ...
                if (l_longFormat) {
                    l_stringFormat = days==1?NSLocalizedStringFromTable(@"%ld day", @"GustyAppKitLocalizable", nil):NSLocalizedStringFromTable(@"%ld days", @"GustyAppKitLocalizable", nil);
                }else{
                    l_stringFormat = NSLocalizedStringFromTable(@"%ldd", @"GustyAppKitLocalizable", @"<NUMBER_OF_DAYS>d");
                }
            ...
            if (showHour) {
                ...
                NSString *l_stringFormat = nil;
                if (l_longFormat) {
                    l_stringFormat = hours==1?NSLocalizedStringFromTable(@"%ld hour", @"GustyAppKitLocalizable", nil):NSLocalizedStringFromTable(@"%ld hours", @"GustyAppKitLocalizable", nil);
                }else{
                    l_stringFormat = @"%ld";
                }
            ...
            if (showMinute) {
                ...
                NSString *l_stringFormat = nil;
                if (l_longFormat) {
                    l_stringFormat = minutes==1?NSLocalizedStringFromTable(@"%ld minute", @"GustyAppKitLocalizable", nil):NSLocalizedStringFromTable(@"%ld minutes", @"GustyAppKitLocalizable", nil);
                }else{
                    l_stringFormat = @"%ld";
                }
            ...
            if (showSecond) {
                ...
                NSString *l_stringFormat = nil;
                if (l_longFormat) {
                    l_stringFormat = seconds==1?NSLocalizedStringFromTable(@"%ld second", @"GustyAppKitLocalizable", nil):NSLocalizedStringFromTable(@"%ld seconds", @"GustyAppKitLocalizable", nil);
                }else{
                    l_stringFormat = @"%ld";
                }
...
+ (NSString*)decimalHoursStringForStartDate:(NSDate*)aStartDate endDate:(NSDate*)anEndDate calendar:(NSCalendar*)a_calendar{
    ...
    NSString *l_format = NSLocalizedStringFromTable(@"%@ h", @"GustyAppKitLocalizable", @"<NUMBER_OF_HOURS> h");
